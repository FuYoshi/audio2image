...
def image_to_wav(image, output_filename, output_type):
...
def wav_to_image(sound, name):
    wav_to_mp3(sound, 'temp')
    wav_to_pixel('temp.mp3', name)
    os.remove('temp.mp3')
...
def parser():
    ...
    if mode == "ati":
        mp3_to_image(file, filename)
    elif mode == "ita":
        image_to_wav(file, filename, export_type)
